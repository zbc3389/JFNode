...
- 4.可设置积分的名字，总量（最小1000个，最大1000亿个），注入波场的数量（最少10个波场）。
- 5.每次铸造有成本消耗，200个波场铸造一次。
- 6.初始价格按铸造时使用的波场价值来定，比如你铸造1000个，花费价值1000个波场来铸造，每个就是1元的价值，如果用1元价值的波场铸造1000个，每个就是0.0001元的价值。
- 7.通过注入波场的方式可以提升每个积分的价格。
- 如：初始1000个哈哈积分，每个价值0.1元，注入价值1000元的波场后，分配到每个积分，就是提升1元的价值，每个积分的价值就提升1元后的结果就是1.1元，每个积分的单价=一共注入的波场价值/总积分数量。
