# JFNode(飓风节点)
- 这是一个去中心化服务器
- 下载最新节点 https://ipfs.io/ipfs/QmbGV1R4BXh825RyZZpphvYu9FHNVEnCotX1woAENMPGQ5?filename=JFNode_1.0.0.7.zip
- 飓风节点(中国国产的去中心化节点)
...
## 搭建方法
- 1.下载最新节点 
    https://ipfs.io/ipfs/QmbGV1R4BXh825RyZZpphvYu9FHNVEnCotX1woAENMPGQ5?filename=JFNode_1.0.0.7.zip
    下载完毕后，解压，进入中本聪节点文件夹。
...
- 3.推广员续费分级别，青铜+5%、白银+10%、黄金+15%、白金+20%、钻石+25%、大师+30%、宗师+35%，包月包季度包年或永久的价格区别很大。
- 4.你直推的用户兑换钻石或者购买商品后，返给你相应额度的10%的节点积分，平台会周期对所有人的节点积分进行回收，以每个1元钱恒定价格自动周期性回收，暂定为一周，直接给你的波场地址内转入对应价格的波场结算。
...
- 3.包月制，按月扣除钻石，可用节点积分续费。
- 4.可以按特定格式上传自己的广告，设置权重（每次看广告时扣除的价格），提交审核，审核通过后才会被浏览，还要提前兑换足够用的广告积分，每被看一次，消耗你1个广告积分，广告积分不足时，无人可以看到你的广告，并且会提示你尽快充值。
- 波场和节点积分都可以兑换广告积分，节点积分兑换即为销毁。
- 提交广告审核前，兑换足够的广告积分，否则广告审核会失败，至少要兑换可以播放1000次广告的广告积分才可以通过后面的审核，1个广告积分价值1元，例如：广告权重如果设置为10元的话，1000次广告播放就是1万个广告积分，需要兑换1万个广告积分才有可能会通过你后续的广告上传提审。
- 1个广告积分价值1元。
- 5.支持自营链接上传和去中心化上传。
...
## 监管系统
- 你可以成为飓风节点的监管人！
- 监管人可以随时检测飓风节点内的所有一般人看不到的数据，数据全是真实的！
- 开启监管人权限，消耗25000波场，和以往的消费制度一样，包月包季度包年或永久，波场续费，比那些都贵，因为是监管数据的部门，收费肯定高。
- 监管商户数据，商品数据，悬赏数据，去中心化App数据，乃至平台内部的所有数据。
- 根据监管不同的数据，开通每项分别缴纳不同的波场，包月包季度包年或永久，波场续费。
- 不想监管你可以不理睬本系统！没人逼你弄！
